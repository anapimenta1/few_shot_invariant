...
class Finetune_inv_1(FSmethod):
    ...
    def forward(self,
        ...
        dist_loss = 0
        for c in y_s.unique():
            z_s_class_mean = z_s[0, y_s == c].mean(0)
            z_q_class_mean = z_q[0, y_q == c].mean(0)
            dist_loss += torch.sum((z_s_class_mean - z_q_class_mean) ** 2)  # Squared Euclidean distance
        dist_loss /= len(y_s.unique())

        lambda_val = 1
        for i in range(1, self.iter):
            probs_s = classifier(z_s[0]).softmax(-1)
            loss_ce = - (y_s_one_hot * probs_s.log()).sum(-1).mean(-1)
            loss = loss_ce + lambda_val*dist_loss
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            # with torch.no_grad():
